...
async function handle(procedure: string, input: any, request: Request, env: Env) {
  ...
  if (procedure === 'dynamicQr.create') {
    const timestamp = now();
    const record = { id: id(), user_id: user.id, slug: input.slug, label: input.label, destination: input.destination, active: input.active === false ? 0 : 1, created_at: timestamp, updated_at: timestamp };
  ...
  if (procedure === 'dynamicQr.update') {
    const existing = await env.DB.prepare('SELECT id FROM dynamic_links WHERE id = ?1 AND user_id = ?2 LIMIT 1').bind(String(input.id), user.id).first();
    if (!existing) throw Object.assign(new Error('Dynamic QR link not found.'), { code: 'NOT_FOUND', status: 404 });
    const fields: string[] = []; const values: unknown[] = [];
    for (const key of ['label', 'destination', 'active'] as const) if (input[key] !== undefined) { fields.push(`${key} = ?`); values.push(key === 'active' ? (input[key] ? 1 : 0) : input[key]); }
    fields.push('updated_at = ?'); values.push(now(), String(input.id), user.id);
    await env.DB.prepare(`UPDATE dynamic_links SET ${fields.join(', ')} WHERE id = ? AND user_id = ?`).bind(...values).run();
    return env.DB.prepare('SELECT id, slug, label, destination, active, created_at AS createdAt, updated_at AS updatedAt FROM dynamic_links WHERE id = ?1').bind(String(input.id)).first();
  }
  if (procedure === 'dynamicQr.remove') {
    await env.DB.prepare('DELETE FROM dynamic_links WHERE id = ?1 AND user_id = ?2').bind(String(input.id), user.id).run();
    return { success: true };
  }
  ...
  if (procedure === 'team.create') {
    ...
    await env.DB.batch([
      env.DB.prepare('INSERT INTO teams (id, name, owner_user_id, created_at, updated_at) VALUES (?1, ?2, ?3, ?4, ?4)').bind(teamId, input.name, user.id, timestamp),
  ...
  if (procedure === 'team.invite') {
    ...
    const memberId = id(); const timestamp = now(); const email = String(input.email).toLowerCase();
    await env.DB.prepare('INSERT INTO team_members (id, team_id, email, role, status, created_at, updated_at) VALUES (?1, ?2, ?3, ?4, \'pending\', ?5, ?5) ON CONFLICT(team_id, email) DO UPDATE SET role = excluded.role, status = \'pending\', updated_at = excluded.updated_at').bind(memberId, teamId, email, input.role, timestamp).run();
    return { email, role: input.role, status: 'pending' };
  }
  if (procedure === 'team.updateRole') {
    if (!canManage(current.role)) throw Object.assign(new Error('FORBIDDEN'), { code: 'FORBIDDEN', status: 403 });
    const result = await env.DB.prepare('UPDATE team_members SET role = ?1, updated_at = ?2 WHERE id = ?3 AND team_id = ?4').bind(input.role, now(), String(input.memberId), teamId).run();
  ...
  if (procedure === 'team.shareLink') {
    ...
    const owned = await env.DB.prepare('SELECT id FROM dynamic_links WHERE id = ?1 AND user_id = ?2 LIMIT 1').bind(String(input.dynamicLinkId), user.id).first();
    if (!owned) throw Object.assign(new Error('NOT_FOUND'), { code: 'NOT_FOUND', status: 404 });
    await env.DB.prepare('INSERT INTO dynamic_link_shares (id, team_id, dynamic_link_id, granted_by_user_id, created_at) VALUES (?1, ?2, ?3, ?4, ?5) ON CONFLICT(team_id, dynamic_link_id) DO UPDATE SET granted_by_user_id = excluded.granted_by_user_id').bind(id(), teamId, String(input.dynamicLinkId), user.id, now()).run();
    return { teamId, dynamicLinkId: String(input.dynamicLinkId), shared: true };
  }
  if (procedure === 'team.unshareLink') {
    if (!canShare(current.role)) throw Object.assign(new Error('FORBIDDEN'), { code: 'FORBIDDEN', status: 403 });
    const result = await env.DB.prepare('DELETE FROM dynamic_link_shares WHERE team_id = ?1 AND dynamic_link_id = ?2').bind(teamId, String(input.dynamicLinkId)).run();
    return { teamId, dynamicLinkId: String(input.dynamicLinkId), removed: result.meta.changes > 0 };
  }
