      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[.85fr_1.15fr]">
        <section className="paper-panel rounded-3xl border p-6 md:p-8">
          ...
          <div className="mt-6 grid gap-3">
            {teams.isLoading ? <p className="text-sm text-[#6f746d]">Loading teams…</p> : teams.error ? <div className="rounded-xl bg-[#fff0eb] p-3 text-sm text-[#9d321b]"><p>Unable to load teams. Please sign in again.</p><button type="button" className="mt-3 rounded-lg border border-[#9d321b] px-3 py-2 font-mono text-[.68rem] uppercase tracking-wider text-[#9d321b]" onClick={() => startLogin({ force: true })}>Sign in with Google</button></div> : teams.data?.length ? teams.data.map(team => <button key={team.id} onClick={() => { clearMessages(); setSelectedTeamId(String(team.id)); }} className={`rounded-2xl border p-4 text-left transition ${team.id === activeTeamId ? 'border-[#0e5b5d] bg-[#dce9e5]' : 'border-[#d7d0c4] bg-[#fffdf8]'}`}><span className="font-display text-lg font-bold">{team.name}</span><span className="mt-1 block font-mono text-xs uppercase tracking-wider text-[#6f746d]">{team.role} · {team.status}</span></button>) : <p className="rounded-2xl border border-dashed border-[#b7b0a3] p-4 text-sm text-[#6f746d]">No team yet. Create one to start sharing.</p>}
          </div>
